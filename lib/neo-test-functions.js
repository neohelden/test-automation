;() => {
  ...
  const showsExpandable = ({
    ...
    if (title)
      fuzzyDataSearchTest(restrictedParticle.response.content, title, 'title')
    if (html)
  ...
  const showsMap = ({ lat = null, lng = null, position = null } = {}) => {
    ...
    if (lat)
      fuzzyDataSearchTest(restrictedParticle.response.content, lat, 'lat')
    if (lng)
      fuzzyDataSearchTest(restrictedParticle.response.content, lng, 'lng')
  }
  ...
  const triggersAudio = (url, { position = null } = {}) => {
    const restrictedParticle =
      position && Number.isInteger(position)
        ? adaptSearchRange(_.cloneDeep(particle), 'directives', position)
        : particle
  ...
  const triggersCamera = ({
    ...
    if (mode)
      fuzzyDataSearchTest(restrictedParticle.response.content, mode, 'mode')
  }

  const triggersStickyClear = (todo, { position = null } = {}) => {
  ...
  const triggersUpload = (target, { position = null } = {}) => {
    ...
    fuzzyDataSearchTest(
      restrictedParticle.response.content.data.target,
      target,
      'target'
    )
  }
   ...
   * @param {Number} [position] nth Element to check(Optional)
   */
  const doesCopyToClipboad = (text, { position = null } = {}) => {
    const restrictedParticle =
      position && Number.isInteger(position)
        ? adaptSearchRange(_.cloneDeep(particle), 'directives', position)
        : particle
  ...
  const doesComposeAnEmail = ({
    ...
    if (recipients)
      fuzzyDataSearchTest(
        restrictedParticle.response.directives,
        recipients,
        'recipients'
      )
    if (subject)
    ...
    if (body)
      fuzzyDataSearchTest(restrictedParticle.response.directives, body, 'body')
  }
   ...
   * Content types with speak attributes: AdaptiveCard, Media, Map, Expandable, Image, Plain/Text, Html, Download
   * @param {String} [position] of the element that says the text(optional)
   */
  const says = (text, { contentType = null, position = null } = {}) => {
    const speakContentTypes = [
      'AdaptiveCard',
      'Media',
      'Map',
      'Expandable',
    ...
    const restrictedParticle =
      position && Number.isInteger(position)
        ? adaptSearchRange(_.cloneDeep(particle), 'content', position)
        : particle
    if (speakContentTypes.includes(contentType))
      isContentType(contentType, { particle: restrictedParticle })
  ...
  const adaptSearchRange = (particle, typeToAdapt, elementToSearch) => {
    ...
    const path = `particle.response.${typeToAdapt}.slice(${
      elementToSearch - 1
    }, ${elementToSearch})`
